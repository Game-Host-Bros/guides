...
## Setting Yourself As An Admin In 7D2D
...
4. Enter this command, Name is an optional identifier.

   ```  
   admin add <ID> <0-1000> <Name>
   ```
   **A permission level of 0 gives the user access to all commands.**
   - Type `admin list` to list admins.
   - To remove an admin, type `admin remove ID`. 
</Steps>

Now that you're an Admin, you may want to check out our guide about [Enabling Admin Menus](/games/7-days-to-die/admin-menus/).

## [Advanced] 7D2D Admin Permission Level Configuration
