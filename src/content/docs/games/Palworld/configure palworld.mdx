...
1. Firstly, login to the [Game Host Bros Panel](https://panel.gamehostbros.com/) and once there, select your Palworld Service
...
:::danger
Using this file will allow you to modify everything, but it will overwrite anything inside PalWorldSettings.ini. This also means if you change the server settings by our panel it will no longer work and you **MUST** update them though WoldOptions.sav.

You also must fill out all the options such as IP, Port, RCON, password etc correctly otherwise you server will not be discoverable/joinable.
:::
