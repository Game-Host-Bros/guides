...
description: Learn how to add admins to your Valheim dedicated server.
sidebar:
  label: Adding Admins
---
...
Managing player permissions—including granting administrative privileges or banning players—is essential for maintaining a healthy gaming environment. [Valheim](https://www.gamehostbros.com/valheim-server-hosting/) uses a file called `adminlist.txt` to handle player permissions.

## Setting Yourself As An Admin In Valheim
...
5. Add a new entry on its own line in [SteamID64](https://steamid.io/) format. It should look like this: 
   ```xml title="adminlist.txt"
   76561198072786081
   ```
...
The Valheim in-game console is disabled by default, here are the steps to enable console if you wish to use Admin commands while playing on your server.
