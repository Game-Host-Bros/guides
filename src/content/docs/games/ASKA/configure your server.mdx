...
:::tip
Some ASKA settings require your server gameplay mode to be `custom`, this can be changed as `mode` in `server properties.txt` located in the home directory of your server under `Management > File Manager`.
:::
...
:::tip
These settings REQUIRE that your server is set to `custom` mode, this can be changed as `mode` in `server properties.txt` located in the home directory of your server under `Management > File Manager`.
:::
