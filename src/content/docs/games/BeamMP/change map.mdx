...
description: Learn how to change your server map on your BeamMP server.
...
To use custom maps on your BeamMP server, you'll need to upload them via SFTP. Follow these steps:

1. Log in to the [Game Host Bros Panel](https://panel.gamehostbros.com/) and select your BeamMP server.
2. Access your server files using SFTP. If you're unfamiliar with SFTP, check out our [SFTP guide](https://guides.gamehostbros.com/getting-started/uploading-files/) for detailed instructions.
3. Upload your custom map files to the `/levels/` directory on your server.
4. After uploading, go to `Configuration > Startup Parameters` in the Game Host Bros Panel.
5. Find the `Map` setting and enter the filepath for your custom map, e.g., `/levels/custom_map_name/info.json`.
6. Restart your server to apply the changes.

:::note
Ensure your custom map files are correctly placed in the `/levels/` directory and that the `info.json` file exists in the map folder.
:::