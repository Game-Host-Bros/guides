...
:::note
If you're using a plugin such as BeamJoy to control map changes, you **must** set `Default Map` to `Disable Start Map` in Configuration > Startup Parameters, doing so will mean our panel will no longer control the map setting.
:::