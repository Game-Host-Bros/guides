...
## Stuck On Connecting (General Mod Troubleshooting)
...
If this doesn't resolve the issue, temporarily rename the `Resources/Client` folder on your server and then restart, doing this will disable mods on your server, then try connecting to your server without mods, If successful you've confirmed it's a mod related issue. To troubleshoot, start by removing/disabling half of the mods on your server and then begin testing. We highly recommend using [SFTP](https://guides.gamehostbros.com/getting-started/uploading-files/) to move multiple mods at a time.

### New BeamNG Profile

Do this if the steps above don't resolve the issue.

- Open the BeamMP launcher.
- Click `Manage User Folder`.
- Select `Open User Folder`.
- Rename the latest folder, for example `0.35` to `0.35_OLD`.
- Start BeamMP and connect to your server.

Still having trouble? Contact us so we can take a look, here's our [Discord](https://discord.gg/FN4XgbAwzr).
