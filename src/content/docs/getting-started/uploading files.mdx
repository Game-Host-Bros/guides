---
title: How to Upload Files to Your Game Server
description: Learn how to upload files to your game server.
sidebar:
  label: Uploading Files
---
import { Steps } from '@astrojs/starlight/components';

This guide will walk you through the process of uploading files on your game server using the File Manager or [Filezilla](https://filezilla-project.org/download.php?show_all=1).

## Using SFTP :badge[Recommended]{variant=success}

This method is suitable if you're uploading folders, multiple files or large files. You will need a program like [FileZilla](https://filezilla-project.org/download.php?show_all=1) to do this.

<Steps>
1. Load the [Game Host Bros Panel](https://panel.gamehostbros.com/) and select your server.
2. In the left-hand menu, click `System > SFTP Details`.
3. Open [FileZilla](https://filezilla-project.org/download.php?show_all=1) and locate the `Quickconnect` bar.
![Game Host Bros Uploading Files](../../../assets/guide-images/filezilla.png)
4. Enter the SFTP connection address you obtained from the game panel into the `Host` field. The port should automatically be set to `2022`.
5. Input the `Username` from the game panel and fill out your `Password`. The password is the same you use to log in to the Game Panel.
6. Click the `Quickconnect` button.
</Steps>

Once connected, you'll see your local files on the left side of the FileZilla window and your game server files on the right side. To upload files, simply locate the files on your local machine (left side), and drag and drop them to the desired location on your game server (right side).

## Using the File Manager

Using the File Manager is suitable for when you need to upload a smaller, single file (up to 1GB) or edit an existing config file.

<Steps>
1. Load the [Game Host Bros Panel](https://panel.gamehostbros.com/) and select your server.
2. In the left-hand menu, click `Management > File Manager`.
3. Click `Upload`, or drag the desired file from your PC into the File Manager.
</Steps>


