...
:::tip
If you need your server moved to a different location, our staff can assist you with that. We can also suggest ideal locations for all your player base.
:::
...
<Steps>
1. Download WinMTR from [here](https://sourceforge.net/projects/winmtr/).
2. Enter the server IP and click Start.
3. Run the test for a few minutes, then click Stop and save the report.
4. Send the `.TXT` report in a [support ticket](http://portal.gamehostbros.com/) to our staff.
</Steps>
We can then send this data to our network provider, but please be aware, **we cannot promise a resolution will come from this, only that they will look into it**.

**If everyone on the server experiences occasional high ping spikes**, this could be due to another server installing, backups being performed or an attack being mitigated. Such **brief blips are normal and usually temporary**. However, if high ping or packet loss is consistently affecting all players on the server for an extended period, please contact our support team to see what we can do for you. 

You can reach us on [Discord](https://discord.gg/FN4XgbAwzr) or submit a [support ticket](http://portal.gamehostbros.com/).